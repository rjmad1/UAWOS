...
STRATEGIC_THEMES = {
    "Theme-1": {
        "name": "Foundation (Individuals & Small Teams)",
        "desc": "Establish local workspaces, objective engines, and core models.",
        "weight": 0.25,
    },
    "Theme-2": {
        "name": "Professional (Departments)",
        "desc": "Integrate memory engines, RAG pipelines, and team collaboration frameworks.",
        "weight": 0.25,
    },
    "Theme-3": {
        "name": "Enterprise (Large Organizations)",
        "desc": "Enforce strict security scans, licenses, Rego policies, and budget controls.",
        "weight": 0.25,
    },
    "Theme-4": {
        "name": "Autonomous Enterprise (Strategic)",
        "desc": "Enable digital twin simulations, adaptive autonomy, and value optimization.",
        "weight": 0.25,
    },
}
...
def ingest_requirement(text: str, llm_enrichment: dict = None) -> dict:
    ...
    parsed_scope = (
        ...
        if llm_enrichment
        else ["General capability deployment"]
    )
    parsed_constraints = (
        llm_enrichment.get("constraints", ["Requires standard UAWOS ports and venv"])
        if llm_enrichment
        else ["Requires UAWOS governance approval"]
    )
        ...
        sequencing_changes.append(
            {
                ...
                "milestone_impact": (
                    "On Schedule"
                    if total_effort <= MAX_PORTFOLIO_CAPACITY
                    else "Milestone Delayed (Capacity Exhausted)"
                ),
            }
        )
        ...
        "dependency_changes": [
            {
                ...
                "name": "Haystack RAG framework integration",
                "type": "Technical",
                "status": "Linked",
            },
            {
                "dependency_id": "DEP-02",
                "name": "Postgres metadata DB schema mapping",
                "type": "Infrastructure",
                "status": "Linked",
            },
        ],
