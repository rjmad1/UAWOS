...
def allocate_objective_budget(objective_id: str, name: str, budget: float) -> dict:
    """Allocate budget for a specific objective.

    Creates or updates the entry in ``objective_budgets`` with the provided ``budget``
    and ``name``. The function loads the persisted state, updates it, saves the state
    back to disk and returns the updated objective dictionary.
    """
    state = load_state()
    # Ensure the objective entry exists
    if objective_id not in state["objective_budgets"]:
        state["objective_budgets"][objective_id] = {
            "budget": budget,
            "actual": 0.0,
            "name": name,
            "status": "Pending",
...
def record_agent_cost(
    ...
    if model_name not in state["token_consumption"]:
        state["token_consumption"][model_name] = {
            "input_tokens": 0,
            "output_tokens": 0,
            "reasoning_tokens": 0,
            "cost": 0.0,
    ...
    if "OBJ-103" in state["objective_budgets"]:
        state["objective_budgets"]["OBJ-103"]["actual"] = round(
            state["objective_budgets"]["OBJ-103"]["actual"] + run_cost, 2
        )
