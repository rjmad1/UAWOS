...
def call_api(prompt, options, context):
    ...
    if "coder" in prompt_id or "coding" in prompt_raw:
        ...
        return {"output": f"Mock Coder Response: Here is the Python implementation for the task: '{task}'."}
    elif "planner" in prompt_id or "plan" in prompt_raw:
        objective = context.get("vars", {}).get("objective", "")
        return {"output": f"Mock Planner Response: Here is the step-by-step plan to achieve: '{objective}'."}
        ...
        return {"output": "Mock System Response: UAWOS Core Coordinator initialized and ready."}
    else:
        return {"output": "Mock Default Response."}
