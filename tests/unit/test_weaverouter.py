...
def test_weaverouter_nvidia_missing_key(monkeypatch):
    ...
    mock_response.read.return_value = (
        b'{"response": "Fallback Ollama response", "prompt_eval_count": 5, "eval_count": 10}'
    )
    mock_response.__enter__.return_value = mock_response

    with patch("urllib.request.urlopen", return_value=mock_response) as mock_urlopen:
        res = uawos_weaverouter.uawos_generate_response(prompt="Test prompt", provider="nvidia", model="tinyllama")
        ...
        state = uawos_observability.load_state(
            state_file=uawos_observability.STATE_FILE, default_state_func=uawos_observability.get_default_state
        )
...
def test_weaverouter_openrouter_missing_key(monkeypatch):
    ...
    mock_response = MagicMock()
    mock_response.read.return_value = (
        b'{"response": "Fallback Ollama response 2", "prompt_eval_count": 2, "eval_count": 3}'
    )
    mock_response.__enter__.return_value = mock_response

    with patch("urllib.request.urlopen", return_value=mock_response) as mock_urlopen:
        res = uawos_weaverouter.uawos_generate_response(prompt="Test prompt", provider="openrouter", model="tinyllama")
...
def test_weaverouter_nvidia_success():
    ...
    with patch("urllib.request.urlopen", return_value=mock_response) as mock_urlopen:
        res = uawos_weaverouter.uawos_generate_response(
            prompt="Test prompt", provider="nvidia", api_key="mock_key", model="meta/llama3-70b-instruct"
        )
...
def test_weaverouter_default_fallback_to_ollama():
    ...
    with patch("urllib.request.urlopen", side_effect=urlopen_side_effect):
        res = uawos_weaverouter.uawos_generate_response(prompt="Test prompt", provider=None)
