...
def uawos_generate_response(
    ...
    if provider == "nvidia":
        try:
            ...
            import sys
            print(f"[NVIDIA NIM Gateway Error] Failed to route request: {e}. Falling back to Ollama...", file=sys.stderr)
            try:
                import uawos_observability
                uawos_observability.emit_telemetry("gateway_fallback", {
                    "provider": "nvidia",
                    "error": str(e),
                    "model": model
                })
        ...
        try:
            ...
            import sys
            print(f"[OpenRouter Gateway Error] Failed to route request: {e}. Falling back to Ollama...", file=sys.stderr)
            try:
                import uawos_observability
                uawos_observability.emit_telemetry("gateway_fallback", {
                    "provider": "openrouter",
