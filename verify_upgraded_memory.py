...
def test_stm_apis():
    ...
    uawos_memory.update_agent_scratchpad(
        "Executor Agent", session_id, "Drafting SQL query improvements...", "optimization_step"
    )
    ...
    assert scratchpad.get("thought_process") == "Drafting SQL query improvements...", (
        "Scratchpad thought process mismatch"
    )
...
def test_episodic_apis():
    ...
    uawos_memory.add_episode_event(
        episode_id, "Executor Agent", "action_run", "Executed test suite checkout queries.", {"latency_ms": 3200}
    )
    uawos_memory.add_episode_event(
        episode_id,
        "Executor Agent",
        "action_failure",
        "Checkout connection query timeout.",
        {"error": "TimeoutException"},
    )

    # 3. Add Decision Records
    uawos_memory.add_episode_decision(
        episode_id,
        "REC-101",
        "Alt B: Cached query optimizations",
        "Allows faster query retrievals in parallel.",
        {"risk_shift": "lowered"},
    )
...
def test_concurrency_advisory_locks():
    ...
    assert execution_order == ["ThreadA_entered", "ThreadA_exited", "ThreadB_entered", "ThreadB_exited"], (
        f"Concurrency mismatch: {execution_order}"
    )
...
def test_hybrid_search():
    ...
    uawos_db.index_knowledge(
        "KNW-301",
        "OAuth Spec Detail",
        "Use OAuth 2.0 client credentials for machine authorization.",
        "document",
        "Security Spec PDF",
    )
    uawos_db.index_knowledge(
        "KNW-302",
        "Checkout Latency Specs",
        "Checkout API connection latency must stay under 100ms threshold.",
        "document",
        "Performance Spec PDF",
    )
...
def test_reflection_and_consolidation(episode_id):
    ...
    uawos_db.index_knowledge(
        "KNW-401", "Redis Config Specs", "Ensure Redis container runs with maxmemory 2gb policy.", "document", "infra"
    )
    uawos_db.index_knowledge(
        "KNW-402",
        "Redis Config Specs v2",
        "Ensure Redis container runs with maxmemory 2gb policy.",
        "document",
        "infra",
    )
