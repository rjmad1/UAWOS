...
def get_default_state() -> dict:
    return {
        "claims": {
            "CLM-01": {"id": "CLM-01", "content": "Query indexes will reduce checkout API latency under 100ms.", "status": "asserted"}
        },
        "evidence": {
            "EVI-01": {"id": "EVI-01", "claim_id": "CLM-01", "source": "uawos_dtase", "text": "Heuristics show 40% cart abandonment on late shipping reveal."}
        },
        "recommendations": {
            "REC-01": {
                "id": "REC-01",
                ...
                "alternatives": ["Alt A: Full checkout rewrite", "Alt B: Cached query optimizations"],
...
def load_state() -> dict:
    state = uawos_db.db_get_state("uawos_decision", None)
...
def save_state(state: dict):
    ...
    uawos_db.db_save_state("uawos_decision", state)
# Core API
