...
# Test without token
req = urllib.request.Request("http://127.0.0.1:8099/api/requirement/submit", method="POST", data=b"{}", headers={"Content-Type": "application/json"})
try:
...
# Test with token
req_secure = urllib.request.Request("http://127.0.0.1:8099/api/requirement/submit", method="POST", data=b"{}", headers={"X-UAWOS-Token": "uawos-secure-token-2026", "Content-Type": "application/json"})
try:
